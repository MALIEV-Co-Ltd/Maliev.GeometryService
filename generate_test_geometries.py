...
# Output directory
output_dir = os.path.join(os.path.dirname(__file__), "tests", "assets")
...
def create_helical():
    ...
    mesh = trimesh.creation.torus(major_radius=1.0, minor_radius=0.3, major_segments=32, minor_segments=16)
    ...
    rotated_vertices = np.column_stack([
        vertices[:, 0] * np.cos(angles * twist_factor) - vertices[:, 2] * np.sin(angles * twist_factor),
        vertices[:, 1],
        vertices[:, 0] * np.sin(angles * twist_factor) + vertices[:, 2] * np.cos(angles * twist_factor)
    ])
...
def main():
    ...
    for filename, mesh in geometries.items():
        ...
        print(f"Created: {filepath} (vertices: {len(mesh.vertices)}, faces: {len(mesh.faces)})")
